...
const availableRoomsFromAPI = ref([]);
...
const filteredRooms = computed(() => {
  if (selectedRoomType.value === 'all') {
    return availableRoomsFromAPI.value;
  }
  return availableRoomsFromAPI.value.filter(room => room.roomType === selectedRoomType.value);
});

// 已选房间信息
const selectedRoomInfo = computed(() => {
  return availableRoomsFromAPI.value.find(r => r.roomId === form.value.roomId);
});
...
// 查询可用房间
async function queryAvailableRooms() {
  try {
    isLoadingRooms.value = true;
    const params = {
      checkInDate: form.value.checkInDate,
      checkOutDate: form.value.checkOutDate,
      roomType: selectedRoomType.value === 'all' ? undefined : selectedRoomType.value
    };

    availableRoomsFromAPI.value = await store.fetchAvailableRooms(params);
  } catch (error) {
...
// 监听日期和房型变化，重新查询可用房间
watch(
  [() => form.value.checkInDate, () => form.value.checkOutDate, selectedRoomType],
  () => {
    queryAvailableRooms();
  }
);
